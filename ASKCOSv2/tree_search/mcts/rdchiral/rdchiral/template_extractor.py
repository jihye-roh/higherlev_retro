...
def remove_different_atom_maps(reactants, products):
    """Only keep atom mapping if both conditions are satisfied
        1) atom map number exists in both reactants and products
        2) atomic number is the same for the atoms with the same atomic num"""
    react_atomicnum = [(atom.GetAtomMapNum(), atom.GetAtomicNum()) for r in reactants for atom in r.GetAtoms() if atom.GetAtomMapNum()]
    prod_atomicnum = [(atom.GetAtomMapNum(), atom.GetAtomicNum()) for p in products for atom in p.GetAtoms() if atom.GetAtomMapNum()]

    react_remove = set(react_atomicnum)- set(prod_atomicnum)
    prod_remove = set(prod_atomicnum) - set(react_atomicnum)
    remove_reactant_maps = [x[0] for x in react_remove if str(x[0])]
    remove_product_maps = [x[0] for x in prod_remove if str(x[0])]
    [x.SetAtomMapNum(0) for r in reactants for x in r.GetAtoms() if x.GetAtomMapNum() in remove_reactant_maps]
    [x.SetAtomMapNum(0) for p in products for x in p.GetAtoms() if x.GetAtomMapNum() in remove_product_maps]
...
def atoms_are_different(atom1, atom2):
    ...
    if atom1.GetIsotope() != atom2.GetIsotope(): return True # ADDED in v2 to detect changes in isotopes as "changes"
...
def set_isotope_to_equal_mapnum(mol):
    for a in mol.GetAtoms():
        if KEEP_ISOTOPE: a.SetProp("Isotope", str(a.GetIsotope()))
...
def clear_isotope(mol):
    ...
    [a.SetIsotope(int(a.GetProp("Isotope"))) if KEEP_ISOTOPE else a.SetIsotope(0) for a in mol.GetAtoms()]

def get_changed_atoms(reactants, products):
    '''Looks at mapped atoms in a reaction and determines which ones changed'''

    if VERBOSE: print('Starting get_changed_atoms...')
    if VERBOSE: print("Reactants: ", [Chem.MolToSmiles(r) for r in reactants])
    if VERBOSE: print("Products: ", [Chem.MolToSmiles(p) for p in products])
    err = 0
    ...
    # print("Changed_atom_tags", changed_atom_tags)
    ...
    if VERBOSE: 
        print("Right before returning changed atoms")
        print("Reactants: ", [Chem.MolToSmiles(r) for r in reactants])
        print("Products: ", [Chem.MolToSmiles(p) for p in products])

    # print("Returning", changed_atoms, changed_atom_tags, err)
...
def convert_atom_to_wildcard(atom):
    ...
    # Initialize
    symbol = '['
    if atom.GetIsotope(): symbol += '{}'.format(atom.GetIsotope())
...
def reassign_atom_mapping(transform):
    ...
    if VERBOSE: print('Reassigned atom mapping from {} to {}'.format(transform, transform_newmaps))
...
def get_fragments_for_changed_atoms(mols, changed_atom_tags, radius=0, 
    ...
    for mol in mols:
        ...
        for i, symbol in enumerate(symbols):
            if re.findall(r'\[\d+', symbol):
                symbols[i] = re.sub(r"\[\d+", "[", symbol)
                isotope_symbol_replacements.append((symbol, symbols[i])) # (original, temp), also include the reverse of stereochemistry
                if '@@' in symbol:
                    isotope_symbol_replacements.append((symbol.replace('@@', '@'), symbols[i].replace('@@', '@')))
                elif '@' in symbol:
                    isotope_symbol_replacements.append((symbol.replace('@', '@@'), symbols[i].replace('@', '@@')))
                if VERBOSE: print('Isotope symbol replacement:', isotope_symbol_replacements)
        if not atoms_to_use: 
            continue
        ...
        num_tetra_flips = 0
        while not tetra_consistent and num_tetra_flips < 200:
            ...
            if VERBOSE: print('Fragment for changed atoms:', this_fragment)
            ...
            map_to_id = {}
            for atom in mol.GetAtoms():
                #if KEEP_ISOTOPE: atom.SetProp("Isotope", str(atom.GetIsotope()))
                if atom.HasProp('molAtomMapNumber'):
                    atom.SetIsotope(int(atom.GetProp('molAtomMapNumber')))
                    map_to_id[atom.GetProp('molAtomMapNumber')] = atom.GetIdx()
                
            
            if VERBOSE: print("Map to ID:", map_to_id)
            ...
            if VERBOSE: 
                ...
                print("Tetra map nums:", tetra_map_nums)
        ...
        for original, temp in isotope_symbol_replacements:
            this_fragment = this_fragment.replace(temp, original)
...
def canonicalize_transform(transform):
    ...
    atom maps.'''
    if VERBOSE: print('Canonicalizing transform...', transform)
    transform_reordered = '>>'.join([canonicalize_template(x) for x in transform.split('>>')])
    if VERBOSE: print('After canonicalization...', transform_reordered)
...
def extract_from_reaction(reaction):
    ...
    """
    Template extractor from https://github.com/connorcoley/rdchiral
    Modified to keep isotope labels, remove atom mapping for reactant atoms that do not end up in theproduct
    """
    if VERBOSE: print("Input reaction: ", reaction)
    reactants_smi = reaction['reactants']
    products_smi = reaction['products']
    # remove_reactant_maps = set(get_atom_mapping(reactants_smi)) - set(get_atom_mapping(products_smi))
    # if remove_reactant_maps:
    #     pattern = '|'.join(r':{}(\]|$)'.format(atom_map) for atom_map in remove_reactant_maps)
    #     # Performing the substitution in one go
    #     reactants_smi = re.sub(pattern, r']', reactants_smi)

    if VERBOSE: print("Reactants SMILES: ", reactants_smi)
    if VERBOSE: print("Products SMILES: ", products_smi)

    reactants = mols_from_smiles_list(replace_deuterated(reactants_smi).split('.'))
    products = mols_from_smiles_list(replace_deuterated(products_smi).split('.'))
    ...
    # Only keep atom mapping if 1) atom map number exists in both reactants and products and 2) atomic number is the same for the atoms with the same atomic num
    remove_different_atom_maps(reactants, products)
    ...
    if GENERALIZE_ISOTOPE:
        react_isotopes = [(atom.GetAtomMapNum(), atom.GetIsotope()) for r in reactants for atom in r.GetAtoms() if atom.GetIsotope()]
        prod_isotopes = [(atom.GetAtomMapNum(), atom.GetIsotope()) for p in products for atom in p.GetAtoms() if atom.GetIsotope()]

        common_isotopes = set(react_isotopes).intersection(set(prod_isotopes))
        remove_isotopes = [x[0] for x in common_isotopes if str(x[0]) not in changed_atom_tags]
        # Assuming changed_atom_tags is a list
        [x.SetIsotope(0) for r in reactants for x in r.GetAtoms() if x.GetAtomMapNum() in remove_isotopes]
        [x.SetIsotope(0) for p in products for x in p.GetAtoms() if x.GetAtomMapNum() in remove_isotopes]
        
    
    if VERBOSE: print("Reactants: ", [Chem.MolToSmiles(r) for r in reactants])
    if VERBOSE: print("Products: ", [Chem.MolToSmiles(p) for p in products])
    
    if VERBOSE: print("Changed_atom_tags", changed_atom_tags)
    ...
    if KEEP_ISOTOPE:
        reactant_fragments, product_fragments = \
            update_fragment_isotopes(reactant_fragments, product_fragments)

    rxn_string = '{}>>{}'.format(reactant_fragments, product_fragments)
    
    if VERBOSE: print("Rxn_string", rxn_string)
    ...
    # remove_reactant_maps = set(get_atom_mapping(reactants_string)) - set(get_atom_mapping(products_string))
    # pattern = '|'.join(r':{}(\]|$)'.format(atom_map) for atom_map in remove_reactant_maps)

    # # Performing the substitution in one go
    # reactants_string = re.sub(pattern, r'\1', reactants_string)
...
def update_fragment_isotopes(reactant_fragments, product_fragments):


    def extract_numbers(s):
        # Regular expression to match sections within brackets and capture both numbers
        pattern = r'\[(?:[^:\[\]]*?(\d+)?(?:[^:\[\]]*:\s*(\d+))?)\]'
        matches = re.findall(pattern, s)
        
        numbers = []
        for match in matches:
            # match is a tuple of captured groups
            isotope = match[0] if match[0] else 0
            map_num = match[1] if match[1] else 0
            numbers.append((int(isotope), int(map_num)))
        
        return numbers

    
    def get_pattern_for_map_number(map_number, fragments):
        # find the pattern for the map number
        pattern = re.compile(r'(\[[^\]]+\:' + f'{map_number}\])')
        match = pattern.search(fragments)
        return match

    def update_fragments(fragments, map_to_isotope, reference_map_to_isotope):
        for map_num, isotope in map_to_isotope.items():
            if reference_map_to_isotope[map_num] and not isotope:
                atom_pattern = get_pattern_for_map_number(map_num, fragments)[0]
                new_atom_pattern = atom_pattern.replace('[', '[0')
                fragments=fragments.replace(atom_pattern, new_atom_pattern)
        return fragments

    product_map_to_isotope = {
        map_num: isotope 
        for isotope, map_num in extract_numbers(product_fragments) if map_num
    }
    reactant_map_to_isotope = {
        map_num: isotope 
        for isotope, map_num in extract_numbers(reactant_fragments) if map_num
    }

    reactant_fragments = update_fragments(
        reactant_fragments, 
        reactant_map_to_isotope, 
        product_map_to_isotope
    )
    product_fragments = update_fragments(
        product_fragments, 
        product_map_to_isotope, 
        reactant_map_to_isotope
    )
...
def get_atom_mapping(string):
    
    """Takes in a SMARTS or SMILES string and returns all atom mapping in the string (in increasing order)

    Returns:
        tuple: tuple of all atom mapping in the string (in increasing order)
    """
    
    # can also be smiles
    return tuple(sorted(int(x) for x in re.findall(r"\:([0-9]+)\]", string)))



def split_templates(template):
    
    """splits a multi-reaction center template into single reaction center templates for intermolecular reactions   
    
    Returns:
        list: list of single reaction center templates
    """
    
    if '.' not in template['products']: return [template]
        
    products = {}
    reactants = {}
    templates = []
    
    for p in template['products'].split('.'):
        products[get_atom_mapping(p)] = p
    
    for r in template['reactants'].split('.'):
        reactants[get_atom_mapping(r)] = r
    
    
    for p in products.keys():
        
        reactant_templates = []
        for r in reactants.keys():
            if set(r).issubset(set(p)):
                reactant_templates.append(reactants[r])
        
        if reactant_templates:
            t = template.copy()
            new_transform = canonicalize_transform('.'.join(['('+x+')' for x in reactant_templates]) + '>>(' + products[p]+')')
            
            t['reactants'], t['products'] = new_transform.split('>>')
            t['products'] = t['products'][1:-1]
            t['reactants'] = t['reactants'][1:-1].replace(').(','.')
            t['reaction_smarts'] = t['products'] + '>>' + t['reactants']
            templates.append(t)
    ...
    for r in reactants.keys():
        ...
        product_templates = []
        for p in products.keys():
            if set(p).issubset(set(r)) and not set(r).issubset(set(p)):
                product_templates.append(products[p])
        
        if product_templates:
            t = template.copy()
            new_transform = canonicalize_transform('(' + reactants[r]+')>>'+'.'.join(['('+x+')' for x in product_templates]))
            
            t['reactants'], t['products'] = new_transform.split('>>')
            t['products'] = t['products'][1:-1].replace(').(','.')
            t['reactants'] = t['reactants'][1:-1]
            t['reaction_smarts'] = t['products'] + '>>' + t['reactants']
            templates.append(t)
            
    return templates

def remove_product_isotopes(rsmi, keepReactionCenters=False):
    
    r, reagents, p  = rsmi.split('>')
    
    remove_isotope_list ={}
    prods = Chem.MolFromSmiles(p)
    reacts = Chem.MolFromSmiles(r)
    
    react_isotopes = dict([(atom.GetAtomMapNum(), atom.GetIsotope()) for atom in reacts.GetAtoms() if atom.GetIsotope()])
    prod_isotopes = dict([(atom.GetAtomMapNum(), atom.GetIsotope()) for atom in prods.GetAtoms() if atom.GetIsotope()])
    
    common_isotopes = set(react_isotopes.items()).intersection(set(prod_isotopes.items()))
    common_isotopes = [x[0] for x in common_isotopes]
    
    [x.SetIsotope(0) for x in prods.GetAtoms() if x.GetAtomMapNum() in common_isotopes]
    [x.SetIsotope(0) for x in reacts.GetAtoms() if x.GetAtomMapNum() in common_isotopes]
    
    return Chem.MolToSmiles(reacts) + '>'+reagents+'>' + Chem.MolToSmiles(prods)

def extract_from_rsmi(rsmi, id=1):
